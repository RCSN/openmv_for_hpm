...
static uint8_t jpeg_get_pixel_size(uint8_t pixel_format)
{
    return (pixel_format == JPEG_PIXEL_FORMAT_ARGB8888) ? 4 : 2;
}
...
hpm_stat_t jpeg_start_encode(JPEG_Type *ptr, jpeg_job_config_t *config)
{
    ...
    ptr->OUTDMA_CTRL1 = JPEG_OUTDMA_CTRL1_ROWLEN_SET(2);
    ...
    ptr->CFG = JPEG_CFG_CFG_IPATH_SEL_SET(config->in_pixel_format)
                             ...
                             uint32_t length)
{
    ...
    ptr->OUTDMA_MISC = JPEG_OUTDMA_MISC_EN_OUTCNT_MASK
        ...
        | JPEG_OUTDMA_MISC_PACK_DIR_SET(config->byte_order)
        | JPEG_OUTDMA_MISC_INI_OUTCNT_MASK
        | JPEG_OUTDMA_MISC_OUT_DMA_REQ_MASK
        | JPEG_OUTDMA_MISC_OUTDMA2D_MASK;
    ptr->OUTDMABASE = JPEG_OUTDMABASE_ADDR_SET(config->out_buffer);
    ptr->OUTDMA_CTRL0 = JPEG_OUTDMA_CTRL0_TTLEN_SET(total_bytes)
        | JPEG_OUTDMA_CTRL0_PITCH_SET(config->width_in_pixel * jpeg_get_pixel_size(config->out_pixel_format));
    ptr->OUTDMA_CTRL1 = JPEG_OUTDMA_CTRL1_ROWLEN_SET(2);
    ptr->ONXT_CMD = JPEG_ONXT_CMD_ADDR_SET(0x14) | JPEG_ONXT_CMD_OP_VALID_MASK;
    ...
    ptr->CSC_COEF0 = 0x04AB01F0
        | JPEG_CSC_COEF0_YCBCR_MODE_SET(config->enable_ycbcr)
        | JPEG_CSC_COEF0_ENABLE_SET(config->enable_csc);
    ptr->CSC_COEF1 = 0x01980204;
    ptr->CSC_COEF2 = 0x0730079C;

    ptr->CFG = JPEG_CFG_CFG_OPATH_SEL_SET(config->out_pixel_format)
        | JPEG_CFG_JDATA_FORMAT_SET(config->jpeg_format)
        | JPEG_CFG_JPEG_EN_MASK
        | JPEG_CFG_START_MASK
        | JPEG_CFG_MODE_MASK;
