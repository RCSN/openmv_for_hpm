...
{
    uint8_t reg;
    int ret = cambus_readb(&sensor->bus, sensor->slv_addr, COM7, &reg);

    switch (pixformat) {
        case PIXFORMAT_RGB565:
            reg = COM7_SET_FMT(reg, COM7_FMT_RGB);
            ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, DSP_CTRL4, DSP_CTRL4_YUV_RGB);
            break;
        ...
        case PIXFORMAT_BAYER:
            ...
            ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, DSP_CTRL4, DSP_CTRL4_RAW8);
            break;
        default:
            return -1;
    }
    ...
    // Write back register
    return cambus_writeb(&sensor->bus, sensor->slv_addr, COM7, reg) | ret;
}
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    // Write LSBs
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, EXHCH, ((w&0x3) | ((h&0x1) << 2)));

    // Sample VFLIP
    ret |= cambus_readb(&sensor->bus, sensor->slv_addr, COM3, &reg);
    vflip = reg & COM3_VFLIP;
    ret |= cambus_readb(&sensor->bus, sensor->slv_addr, HREF, &reg);
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, HREF, (reg & 0xBF) | (vflip ? 0x40 : 0x00));
    ...
    if ((w <= 320) && (h <= 240)) {
        ...
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, HSTART, 0x3F);
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, HSIZE,  0x50);
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, VSTART, 0x03 - vflip);
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, VSIZE,  0x78);

        // Enable auto-scaling/zooming factors
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, DSPAUTO, 0xFF);
    } else {
        // Set VGA Resolution
        uint8_t reg;
        int ret = cambus_readb(&sensor->bus, sensor->slv_addr, COM7, &reg);
        ...
        // Set VGA Window Size
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, HSTART, 0x23);
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, HSIZE,  0xA0);
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, VSTART, 0x07 - vflip);
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, VSIZE,  0xF0);

        // Disable auto-scaling/zooming factors
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, DSPAUTO, 0xF3);

        // Clear auto-scaling/zooming factors
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, SCAL0, 0x00);
...
static int set_contrast(sensor_t *sensor, int level)
{
    ...
    return cambus_writeb(&sensor->bus, sensor->slv_addr, CONTRAST, contrast_regs[level][0]);
}
...
static int set_brightness(sensor_t *sensor, int level)
{
    ...
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, BRIGHTNESS, brightness_regs[level][0]);
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, SIGN_BIT,   brightness_regs[level][1]);
    return ret;
}
...
static int set_saturation(sensor_t *sensor, int level)
{
    ...
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, USAT, saturation_regs[level][0]);
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, VSAT, saturation_regs[level][1]);
    return ret;
}
...
{
    uint8_t reg;
    int ret = cambus_readb(&sensor->bus, sensor->slv_addr, COM9, &reg);

    // Set gain ceiling
    reg = COM9_SET_AGC(reg, gainceiling);
    return cambus_writeb(&sensor->bus, sensor->slv_addr, COM9, reg) | ret;
}

static int set_colorbar(sensor_t *sensor, int enable)
{
    uint8_t reg;
    int ret = cambus_readb(&sensor->bus, sensor->slv_addr, COM3, &reg);

    // Enable colorbar test pattern output
    reg = COM3_SET_CBAR(reg, enable);
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM3, reg);

    // Enable DSP colorbar output
    ret |= cambus_readb(&sensor->bus, sensor->slv_addr, DSP_CTRL3, &reg);
    reg = DSP_CTRL3_SET_CBAR(reg, enable);
    return cambus_writeb(&sensor->bus, sensor->slv_addr, DSP_CTRL3, reg) | ret;
}

static int set_auto_gain(sensor_t *sensor, int enable, float gain_db, float gain_db_ceiling)
{
    uint8_t reg;
    int ret = cambus_readb(&sensor->bus, sensor->slv_addr, COM8, &reg);
    ...
    if ((enable == 0) && (!isnanf(gain_db)) && (!isinff(gain_db))) {
        ...
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM9, (reg & 0x8F) | ((fast_ceilf(fast_log2(gain_ceiling)) - 1) << 4));
    }
...
static int get_gain_db(sensor_t *sensor, float *gain_db)
{
    ...
    // DISABLED
    // if (reg & COM8_AGC_EN) {
    //     ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM8, COM8_SET_AGC(reg, 0));
    // }
    // DISABLED
    ...
    // DISABLED
    // if (reg & COM8_AGC_EN) {
    //     ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM8, COM8_SET_AGC(reg, 1));
    // }
    // DISABLED
...
static int set_auto_exposure(sensor_t *sensor, int enable, int exposure_us)
{
    uint8_t reg;
    int ret = cambus_readb(&sensor->bus, sensor->slv_addr, COM8, &reg);
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM8, COM8_SET_AEC(reg, (enable != 0)));

    if ((enable == 0) && (exposure_us >= 0)) {
        ret |= cambus_readb(&sensor->bus, sensor->slv_addr, COM7, &reg);

        int t_line = (reg & COM7_RES_QVGA) ? (320 + 256) : (640 + 144);
        int t_pclk = (COM7_GET_FMT(reg) == COM7_FMT_P_BAYER) ? 1 : 2;

        ret |= cambus_readb(&sensor->bus, sensor->slv_addr, COM4, &reg);
        int pll_mult = 0;
...
static int get_exposure_us(sensor_t *sensor, int *exposure_us)
{
    ...
    // DISABLED
    // if (reg & COM8_AEC_EN) {
    //     ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM8, COM8_SET_AEC(reg, 0));
    // }
    // DISABLED

    ret |= cambus_readb(&sensor->bus, sensor->slv_addr, AEC, &aec_l);
    ret |= cambus_readb(&sensor->bus, sensor->slv_addr, AECH, &aec_h);

    // DISABLED
    // if (reg & COM8_AEC_EN) {
    //     ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM8, COM8_SET_AEC(reg, 1));
    // }
    // DISABLED
...
static int set_auto_whitebal(sensor_t *sensor, int enable, float r_gain_db, float g_gain_db, float b_gain_db)
{
    uint8_t reg;
    int ret = cambus_readb(&sensor->bus, sensor->slv_addr, COM8, &reg);
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM8, COM8_SET_AWB(reg, (enable != 0)));
    ...
    if ((enable == 0) && (!isnanf(r_gain_db)) && (!isnanf(g_gain_db)) && (!isnanf(b_gain_db))
        ...
        ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, GREEN, g_gain);
    }
...
static int get_rgb_gain_db(sensor_t *sensor, float *r_gain_db, float *g_gain_db, float *b_gain_db)
{
    ...
    // DISABLED
    // if (reg & COM8_AWB_EN) {
    //     ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM8, COM8_SET_AWB(reg, 0));
    // }
    // DISABLED

    ret |= cambus_readb(&sensor->bus, sensor->slv_addr, BLUE, &blue);
    ret |= cambus_readb(&sensor->bus, sensor->slv_addr, RED, &red);
    ret |= cambus_readb(&sensor->bus, sensor->slv_addr, GREEN, &green);

    // DISABLED
    // if (reg & COM8_AWB_EN) {
    //     ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM8, COM8_SET_AWB(reg, 1));
    // }
    // DISABLED
...
static int set_hmirror(sensor_t *sensor, int enable)
{
    uint8_t reg;
    int ret = cambus_readb(&sensor->bus, sensor->slv_addr, COM3, &reg);
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM3, COM3_SET_MIRROR(reg, enable)) ;
...
static int set_vflip(sensor_t *sensor, int enable)
{
    uint8_t reg;
    int ret = cambus_readb(&sensor->bus, sensor->slv_addr, COM3, &reg);
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, COM3, COM3_SET_FLIP(reg, enable));
...
static int set_special_effect(sensor_t *sensor, sde_t sde)
{
    ...
    switch (sde) {
        case SDE_NEGATIVE:
            ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, SDE, 0x46);
            break;
        case SDE_NORMAL:
            ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, SDE, 0x06);
            ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, UFIX, 0x80);
            ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, VFIX, 0x80);
            break;
        default:
            return -1;
    }
...
static int set_lens_correction(sensor_t *sensor, int enable, int radi, int coef)
{
    ...
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, LC_CTR, (enable&0x01));
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, LC_RADI, radi);
    ret |= cambus_writeb(&sensor->bus, sensor->slv_addr, LC_COEF, coef);
